...
//display all spacecraft details
const ul=document.querySelector("#results-list");
const render=()=>{
ul.textContent="";
for(let item of data){
    ul.insertAdjacentHTML("beforeend",`<li>${item.spacecraft}</li>`);
   
}
}
render();
// form search keyup event 
...
input.addEventListener("keyup",event=>{
    ...
    const value=document.querySelector("#app-search").value;
    ul.textContent="";
    for(let item of data){
        
        if(item.spacecraft.toLowerCase().trim().includes(value)){
